...
| `onchain_offer_id` | `bigint` | On-chain offer ID from the Marketplace contract. Use this as the `offerId` arg to contract calls. |
| `contract_address` | `text` | Lowercase address of the Marketplace contract. Combined with `onchain_offer_id` as a compound unique key — prevents cross-deployment ID collisions. |
...
**Unique constraint:** `(onchain_offer_id, contract_address)` — scopes offer IDs to a specific contract deployment so `created_at` is always set fresh on INSERT, never preserved from a prior deployment's row.
